...
// js/src/icons.js
var svg = (paths) => `<svg class="bootcn-icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths}</svg>`;
...
var Combobox = class _Combobox {
  ...
  constructor(select) {
    ...
    this.searchable = select.hasAttribute("data-bootcn-select") ? false : select.dataset.search !== "false";
    this.active = -1;
    this._build();
  }
  _build() {
    ...
    const search = this.searchable ? `<div class="bootcn-combobox-search-wrap">${SEARCH}<input type="text" class="bootcn-combobox-search" placeholder="${escapeAttr(this.searchPlaceholder)}" autocomplete="off" spellcheck="false"></div>` : "";
    menu.innerHTML = search + `<ul class="bootcn-combobox-list" role="listbox"></ul><div class="bootcn-combobox-empty" hidden>${escapeHtml(this.emptyText)}</div>`;
...
// js/src/toggle.js
var store13 = /* @__PURE__ */ new WeakMap();
var groupStore = /* @__PURE__ */ new WeakMap();
var Toggle = class _Toggle {
  static getOrCreate(el) {
    let inst = store13.get(el);
    if (!inst) {
      inst = new _Toggle(el);
      store13.set(el, inst);
    }
...
var ToggleGroup = class _ToggleGroup {
  static getOrCreate(el) {
    let inst = groupStore.get(el);
    if (!inst) {
      inst = new _ToggleGroup(el);
      groupStore.set(el, inst);
    }
    return inst;
  }
  constructor(el) {
...
// js/src/chart.js
var store14 = /* @__PURE__ */ new WeakMap();
var COLORS = ["--chart-1", "--chart-2", "--chart-3", "--chart-4", "--chart-5"];
function tokenFill(i) {
  return `hsl(var(${COLORS[i % COLORS.length]}))`;
}
var Chart = class _Chart {
  static getOrCreate(el) {
    let inst = store14.get(el);
    if (!inst) {
      inst = new _Chart(el);
      store14.set(el, inst);
    }
  ...
  _render() {
    ...
    let svg2 = "";
    if (this.type === "donut") svg2 = this._donut(pts, max);
    else if (this.type === "line" || this.type === "area") svg2 = this._line(pts, max, this.type === "area");
    else svg2 = this._bar(pts, max);
    this.el.insertAdjacentHTML("afterbegin", svg2);
    if (this.el.dataset.legend !== "false") {
      const legend = pts.map(
        (p, i) => `<span><span class="bootcn-chart-swatch" style="background:${tokenFill(i)}"></span>${escapeHtml(p.label)}</span>`
      ).join("");
  ...
  _bar(pts, max) {
    ...
    const bars = pts.map((p, i) => {
      ...
      const y = pad.t + ih - bh;
      return `<rect x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${bw.toFixed(1)}" height="${bh.toFixed(1)}" rx="3" fill="${tokenFill(i)}"><title>${escapeHtml(p.label)}: ${p.value}</title></rect><text class="bootcn-chart-axis" x="${(x + bw / 2).toFixed(1)}" y="${h - 6}" text-anchor="middle">${escapeHtml(p.label)}</text>`;
    }).join("");
  ...
  _line(pts, max, area) {
    ...
    const areaD = area ? `${d} L${xy[xy.length - 1].x.toFixed(1)},${(pad.t + ih).toFixed(1)} L${xy[0].x.toFixed(1)},${(pad.t + ih).toFixed(1)} Z` : "";
    const dots = xy.map(
      (c, i) => `<circle cx="${c.x.toFixed(1)}" cy="${c.y.toFixed(1)}" r="3.5" fill="${tokenFill(i)}"><title>${escapeHtml(c.p.label)}: ${c.p.value}</title></circle>`
    ).join("");
    const labels = xy.map(
      (c) => `<text class="bootcn-chart-axis" x="${c.x.toFixed(1)}" y="${h - 6}" text-anchor="middle">${escapeHtml(c.p.label)}</text>`
    ).join("");
    const fill = area ? `<path d="${areaD}" fill="hsl(var(--chart-1) / .2)" stroke="none"/>` : "";
    return `<svg viewBox="0 0 ${w} ${h}" role="img" aria-label="${area ? "Area" : "Line"} chart">${fill}<path d="${d}" fill="none" stroke="${tokenFill(0)}" stroke-width="2"/>${dots}${labels}</svg>`;
  }
  _donut(pts) {
    ...
    const rings = pts.map((p, i) => {
      ...
      const el = `<circle cx="${cx}" cy="${cy}" r="${r}" fill="none" stroke="${tokenFill(i)}" stroke-width="${stroke}" stroke-dasharray="${dash}" stroke-dashoffset="${(-offset).toFixed(2)}" transform="rotate(-90 ${cx} ${cy})"><title>${escapeHtml(p.label)}: ${p.value}</title></circle>`;
      offset += len;
      return el;
    }).join("");
